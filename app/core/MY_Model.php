...
class MY_Model extends CI_Model{
	...
	**/
	...
	protected $unencrypted_tables = array(
		'transaction_alerts',
	);
	...
	public function update_table($primary_value, $data, $table, $skip_validation = FALSE)
	{
	...
	*/
	...
	public function select_secure($field,$table=''){
		if($field=='id') {
		...
		}else if($field=='group_id'||in_array($this->_table, $this->unencrypted_tables)){
			if($table){
				$this->db->select("$table".".$field");
			}else{
				$this->db->select("$field");
			...
			if($table){
				$this->db->select(array("AES_DECRYPT($table.$field,"."SHA2('".SALT."',512)".") as $field"));
			}
			else{
				$this->db->select(array("AES_DECRYPT($field,"."SHA2('".SALT."',512)".") as $field"));
			}
		}
	...
	public function secure_data($data,$field=''){
	...
	public function insert_secure($data, $skip_validation = FALSE)
	{
		...
		if($data&&count($data)){
			...
			foreach($data as $k=>$v){
				$query .=$k;
				$query2 .='0x'.$this->secure_data($v,$k);
	...
	function _encrypt_data($value = '',$field_name = ''){
		...
		if($field_name == 'group_id'||in_array($this->_table, $this->unencrypted_tables)){
			if($this->_table == 'transaction_alerts'&&($field_name=='description'||$field_name=='particulars')){
				$value = nl2br($value);
			}
			$value = $value?$this->db->escape_str($value):0;
			return '"'.$value.'"';
			...
			return 'AES_ENCRYPT('.$safe_value.',SHA2("'.SALT.'",512))';
		}
	}
	...
	public function insert_secure_data_ignore($table,$data)
	{
		...
		if($data&&count($data)){
			...
			foreach($data as $k=>$v){
				$query .=$k;
				$query2 .='0x'.$this->_encrypt_data($v,$k);
				if($xc==count($data)) break;
					$query .=', ';
					$query2 .=', ';
				$xc++;
	 ...
	 * @author Samuel Wahome
	 */
	public function update_secure_data($id,$table,$data,$group_id=0){
		//encrypt data
		if($data&&count($data)){
			...
			foreach($data as $k=>$v){
				...
				$query .=' '.$k.' = '.$this->_encrypt_data($v,$k);
	...
	public function update_unencrypted_group_id($table){
		$query = 'update '.$table.' set group_id = '.$this->old_dx('group_id').' ';
		//$query = 'update '.$table.' set group_id = '.$this->dx('group_id').' ';
		return $this->db->query($query);
	}
	...
	public function update_secure_where($where,$table,$data){
		...
		if($data&&count($data)){
			...
			foreach($data as $k=>$v){
				...
				$query .=' '.$k.' = 0x'.$this->secure_data($v,$k);
	...
	public function ex($field){
		...
		if($field == 'group_id'||in_array($this->_table, $this->unencrypted_tables)){
			return $field;
		}else{
	...
	public function dx($field){
		if($field == 'group_id'||stripos(strtolower($field),'.group_id')||in_array($this->_table, $this->unencrypted_tables)){
			return $field;
		}else{
